...
function parseScalarMap(
  lines: string[],
  start: number
): { map: Record<string, number>; next: number } {
...
export function parseStatblockYaml(raw: string): ParsedStatblock {
  ...
  for (let i = 0; i < lines.length;) {
    ...
    if (
      (key === 'legendary_actions' || key === 'legendaryactions' || key === 'legendary') &&
      !value
    ) {
      const parsed = parseNamedList(lines, i + 1);
...
export function extractStatblock(
  markdown: string
): { block: ParsedStatblock; rest: string } | null {
  ...
  if (unfenced) {
    ...
    return {
      ...
      rest: (
        markdown.slice(0, unfenced.index) +
        markdown.slice((unfenced.index ?? 0) + unfenced[0].length)
      ).trim(),
    };
  }
  return null;
...
export function extractTagline(markdown: string): string {
  const heading =
    /^>\s*###\s+\*(.+)\*\s*$/m.exec(markdown) || /^###\s+\*(.+)\*\s*$/m.exec(markdown);
...
export function parsedToBestiaryMarkdown(block: ParsedStatblock): string {
  ...
  if (block.vulnerabilities)
    yaml.push(`damage_vulnerabilities: ${JSON.stringify(block.vulnerabilities)}`);
...
export function parsedToStatBlock(
  block: ParsedStatblock
): import('../../../shared/types').StatBlock {
  return {
    name: block.name,
    size: block.size,
    type: block.type,
