...
const VARIANTS: Record<
  string,
  { label: string; bar: string; box: string; labelClass: string; titleClass: string }
> = {
  tip: {
    label: 'Tip',
    bar: 'bg-amber',
  ...
  return (
    <section className="callout-card my-4">
      <div className={`relative rounded-md border px-4 pb-4 pt-5 ${variant.box}`}>
        <div
          className={`pointer-events-none absolute inset-y-0 left-0 w-1 rounded-l-md ${variant.bar}`}
        />
        <div className="absolute -top-3 left-3 bg-panel px-2">
          <span
            className={`text-[10px] font-semibold uppercase tracking-[0.22em] ${variant.labelClass}`}
          >
            {variant.label}
          </span>
        </div>
        <div className="callout-card-body pl-2">
          {title ? (
            <h3
              className={`!mt-0 font-display text-[1.05rem] ${variant.titleClass} ${children ? '!mb-2' : '!mb-0'}`}
            >
              {title}
            </h3>
          ) : null}
