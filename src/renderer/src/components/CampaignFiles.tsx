...
function displayName(name: string): string {
  return name
    .replace(/\.(md|markdown|txt|json|png|jpe?g|webp|gif|svg|bmp|pdf)$/i, '')
    .replace(/[-_]/g, ' ');
}
...
function TreeNode({
  ...
  return (
    ...
    >
      {kind === 'image' ? (
        <img
          src={campaignFileUrl(node.relativePath)}
          alt=""
...
export default function CampaignFiles({
  ...
  const folderPath =
    menu?.kind === 'node' && menu.node.type === 'dir' ? menu.node.relativePath : '';
  const folderHint =
    menu?.kind === 'node' && menu.node.type === 'dir' ? folderKind(menu.node.name) : null;
