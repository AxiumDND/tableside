    ...
    return splitCalloutBlocks(text).map((part, i) => {
      ...
      if (part.kind !== 'prose') {
        return (
          <CalloutCard
            key={key}
            type={part.kind === 'other' ? (part.type ?? 'note') : part.kind}
            title={part.title}
          >
            {part.markdown.trim() ? (
              <Markdown
    ...
    if (docSections.length === 0) {
      return (
        <div className="markdown-body">
          {renderMarkdown(text || '_This file is empty._', keyPrefix)}
        </div>
      );
    }
    return docSections.map((section, index) => {
      ...
      return (
        <div key={key}>
          ...
          {rest.trim() ? (
            <div className="markdown-body">{renderMarkdown(rest, `${key}-rest`)}</div>
          ) : null}
        </div>
      );
    });
        ...
        <div className="flex items-center justify-between gap-2">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h2 className="min-w-0 truncate font-display text-lg text-amber">
              ...
              {dirty ? (
                <span className="ml-2 text-xs font-sans text-amber-dim">unsaved</span>
              ) : null}
            </h2>
          </div>
          ...
          {!path ? (
            ...
            <div className="flex h-full flex-col items-center justify-center gap-3">
              ...
              <p className="text-xs text-muted">
                Selected — press Show to players to put this on the second monitor.
              </p>
            </div>
          ) : kind === 'pdf' && imageUrl ? (
            <iframe
            ...
            <div className="mx-auto max-w-sm space-y-3">
              ...
              {character.notes ? (
                <p className="text-sm text-parchment/90">{character.notes}</p>
              ) : null}
            </div>
          ) : npcMode && parsedNpc ? (
            <NpcSheet
              ...
              onAddToCombat={
                onAddNpcToCombat ? () => onAddNpcToCombat(parsedNpc.block, path) : undefined
              }
              onEdit={() => setEditing(true)}
              renderNotes={(body) =>
                renderDocument(
                  linkWikiNotes(
                    prepareNoteMarkdown(body, path, images, { injectPortrait: false }),
                    path,
                    noteIndex
                  ),
                  'sheet'
