...
export default function PlayerView({ state, compact }: { state: PlayerState; compact?: boolean }) {
      ...
      {showInit ? (
        <div className="player-init" aria-label="Initiative order">
          <div className="player-init-round">
            {state.initiativeRound && state.initiativeRound > 0
              ? `Round ${state.initiativeRound}`
              : 'Initiative'}
          </div>
          <ol className="player-init-list">
            {state.initiative.map((entry) => (
              ...
              >
                ...
                {entry.condition === 'dead' ? (
                  <span className="player-init-tag is-blood">Dead</span>
                ) : null}
                {entry.condition === 'unconscious' ? (
