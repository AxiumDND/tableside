...
export default function DmApp() {
  ...
  useEffect(() => {
    refresh();
    const cleanup = window.tabledm.onPlayerState(setPlayer);
    return () => {
      cleanup();
    };
  }, [refresh]);
  ...
  function changeRightPanel(
    next: typeof rightPanel | ((prev: typeof rightPanel) => typeof rightPanel)
  ): void {
    setRightPanel((prev) => {
        ...
        <header className="flex items-center gap-3 border-b border-line bg-panel px-4 py-2">
          <div>
            <div className="flex items-baseline gap-2">
              ...
              <div className="text-[11px] font-semibold tracking-wide text-amber-dim">
                v{APP_VERSION}
              </div>
            </div>
          ...
          <div className="ml-4 min-w-0 flex-1">
            ...
            <div className="truncate text-[11px] text-muted">
              {campaign?.folder ?? 'Choose a folder to begin'}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="flex min-h-0 flex-1">
          <div className="flex w-64 shrink-0 flex-col border-r border-line">
            ...
            {campaign ? (
              ...
              <div className="flex-1 bg-ink px-3 py-4 text-xs text-muted">
                Open a campaign to see files.
              </div>
            )}
            <DiceTray />
          </div>
          <SessionNotes
            ...
            kind={openKind}
            imageUrl={
              openKind === 'image' || openKind === 'pdf' ? campaignFileUrl(openPath) : undefined
            }
            ...
            onShowToPlayers={
              selectedImage || openKind === 'image' ? () => void showSelectedToPlayers() : undefined
            }
            onOpenNote={openNote}
