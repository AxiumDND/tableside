...
describe('gear folders', () => {
  ...
  it('keeps top-level Gear in the standard folder order', () => {
    ...
    expect(folderOrderIndex('Sessions')).toBeLessThan(folderOrderIndex('Calendar'))
    expect(folderOrderIndex('Calendar')).toBeLessThan(folderOrderIndex('Party'))
    expect(folderMatchesCanonical('Almanac', 'calendar')).toBe(true)
