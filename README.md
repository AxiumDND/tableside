...
**Table-first, not a VTT.** Tableside is for people sitting together in the same room. It is not meant to be a full virtual tabletop for online play. There is no account, no cloud campaign sync, and no internet required once the app is installed. Current release: **1.8.23**.
...
**[Download the latest installer](https://github.com/AxiumDND/tableside/releases/latest)** — look for `Tableside-Setup-1.8.23.exe`. No account, no admin.
...
4. **Combat** loads a game night sheet’s combatants. **Music** plays your files from `Audio/`. The **Quick** bar holds Lookup (offline system pack), Prep (NPC, Links), Table (Dice, Timer, Improvise), Party, Conditions, and the in-world calendar.
