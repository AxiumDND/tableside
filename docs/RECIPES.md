...
1. Create sheets under `Party/`, `NPCs/`, and `Bestiary/` (right-click the folder, or save from Lookup). For an online character or monster page, open the Party / NPC / Bestiary sheet and use **Add web sheet** — then flip to the live page in the note pane. Copy AC/HP onto the campaign sheet if you want them in Combat.
...
5. **Start** combat, advance turns (`Alt+T`; `Alt+Shift+T` to go back), adjust HP.
...
2. Press **Lookup** on the Quick bar.
