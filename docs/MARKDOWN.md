...
| `crawl` | `opening` | Opening crawl card. Edit title, far-off line, emblem, crawl music, and crawl on the card (writes back to the note). **Play** (Sci-fi look only) shows a starfield, an original far-off line (`preface:`; `none` skips it), a generic title emblem (or the first `![[image]]`), then a perspective prologue. Optional `music: Audio/Music/…` overrides the mood playlist for that track and resumes it when the track ends or you Clear. Write your own words — Tableside does not include licensed crawl text, logos, or music files. |
