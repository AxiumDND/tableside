...
- On **Pan**, **Line** / **Cone** / **Round** plus the second **ft** box drop a spell-area template on that grid (default 30 ft). Line is 5 ft wide; cone is a 90° sector (as wide as it is long). Click origin and drag to aim; **Round** is a radius at the click. Templates are DM-only and not saved. Esc or the same button clears. Shift+scroll while a template is armed nudges feet by 5.
...
- **Pan** is the default: drag to move, scroll or the **Zoom** slider to zoom, **Fit** resets. **Scale map** sets the 5 ft grid. **Line** / **Cone** / **Round** overlay a feet-sized template. **Show to players** follows crop, fog, and tokens — pins stay DM-only.
