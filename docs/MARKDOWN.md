...
| `crawl` | `opening` | Opening crawl card. Edit title, far-off line, emblem, crawl music, and crawl on the card (writes back to the note). **Play** (Sci-fi look only) shows a starfield, an original far-off line (`preface:`; `none` skips it), a generic title emblem (or the first `![[image]]`), then a perspective prologue. Optional `music: Audio/Music/…` fades mood out on Play; the crawl track starts half a second before the emblem, then resumes mood when the track ends, you Stop, or you Clear. Write your own words — Tableside does not include licensed crawl text, logos, or music files. |
