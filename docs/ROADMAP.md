...
7. **One Campaign button** — fold header **New campaign**, **Open campaign**, and **Switch campaign** into a single always-visible **Campaign ▾** menu. Recents (name + path, current folder omitted) at the top; **Open…** and **New…** always at the bottom. Hide the Recent block when there is nothing to switch to — do not hide Open/New. Keep **Sample** on the empty start screen only, not in the header menu. Same menu on the start screen next to Sample. Local recents only — no cloud library. Design sketch: [CAMPAIGN-MENU.md](CAMPAIGN-MENU.md).
