...
| Fence | Use |
| --- | --- |
| `[!party]` | PC + companion links; live race / class / AC / HP table (see above) |
...
| `[!gallery]` / `[!video]` | Player slideshow / local clip |
...
Optional `Templates/` at the campaign root overrides built-in **New …** sheets (`Party Roster.md`, `Session Recap.md`, `Game Night Sheet.md`, `Player.md`, …). The folder is hidden in the file tree. Prefer the built-in 1.7.12 shapes unless the table already has a house template.
