...
2. **Campaign** (header) — one menu. Other recent folders (name + path) are listed first; the open folder is omitted. Missing folders drop off the list when you try them. **Open campaign…** picks any folder on disk. Folders without `"system"` in `campaign.json` run as D&D 5e. **New campaign…** picks a system (D&D 5e, Pathfinder 2e, or Vampire 5th), then a look, then an empty folder; Tableside scaffolds the standard layout. Changing system later is not supported. Theme can change any time from Start Here. The empty start screen uses the same **Campaign** menu next to **Sample**.
