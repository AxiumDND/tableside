...
- **Hooks + components** (`src/renderer/src/**/*.test.tsx`): React Testing Library. Opt into jsdom with a top-of-file `// @vitest-environment jsdom` comment. Prefer plain Vitest matchers (`toBeTruthy()`, `queryByRole(...)` + `toBeNull()`) over `jest-dom`'s `toBeInTheDocument()` — `vitest.setup.ts` registers jest-dom, but existing tests stick to plain assertions.
